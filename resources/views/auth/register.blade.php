<x-guest-layout>
    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div>
            ...
            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" placeholder="e.g. John Doe" />
        ...
        <div class="flex items-center justify-between mt-6">
            <a class="underline text-xs text-slate-500 hover:text-emerald-500 rounded-md focus:outline-none" href="{{ route('login') }}">
        ...
        <div class="mt-4 text-center border-t border-slate-200 dark:border-slate-800/80 pt-3">
            <span class="text-[10px] text-slate-500 font-mono-tech uppercase">Already bought as guest? </span>
            <a class="text-[10px] font-mono-tech uppercase text-emerald-400 hover:text-emerald-300 transition duration-150 underline" href="{{ route('login') }}">
                Sign in with OTP
            </a>
        </div>
    </form>
